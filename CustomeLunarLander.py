...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset_(self, pos_x, pos_y):
        ...
        self.lander.ApplyForceToCenter((
            ...
            self.np_random.uniform(500, 1000)
        ), True)

        self.legs = []
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(pos_x - i * LEG_AWAY / SCALE, pos_y),
                angle=(i * 0.05),
                fixtures=fixtureDef(
        ...
        return self.step(np.array([0, 0]) if self.continuous else 2)[0]
    ...
    def step(self, action):
        ...
        if not self.lander.awake:
            ...
            reward = +100
        return np.array(state, dtype=np.float32), reward, done, {}, self.lander.awake, self.game_over
